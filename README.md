...
> `lvm`快照通过的写时复制策略来完成备份。快照创建的时候所在分区目录是对应逻辑卷
> 生成对应数据节点的硬链接。如果数据没有改变，快照分区只会保留数据的一个硬链接。
> 一旦修改对应数据，`lvm`会自动复制旧数据，并将快照指向旧数据。修改之后的数据暂
> 时保留在原先目录中。
...
-   解压缩对应时间节点的快照压缩包。
    ...
    ```
    tar Jpvxf snaphost<date>.tax.xz
    ```
...
-   复制对应时间节点的快照数据到`/dev/vg0/snap01`

    ```
    rsync -a /opt/backup/snapshot/* /mnt
    ```
...
> 回滚数据的恢复程度受备份时间间隔影响，本文选择一周一次，理论只能到上周六的数据
> 状态。但是，快照即使没有有效数据也会被有充满对应数据的硬链接，如果备份过于频繁
> 也会占用很多空间。
